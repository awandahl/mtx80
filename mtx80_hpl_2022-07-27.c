/*
---------- FOR G3ZOI - TRO-2  -----------
Original Code by VE2EMM                      Date: Jan 2003
First Modified by G3ZOI                    Date: Aug 2006

Comfort transmissions at 5sec intervals
Auto shutdown, set by variable 'timeLimit' Date: Mar 2022
Standby current = 2.2mA in shutdown state.
...
REM OUT THE DEFINES YOU DO NOT WANT
*/
...
// EEPROM LOAD : TX-Number(0=MOi)+CALLSIGN
//#rom 0x2100={0x00,0x06,0x0a,0x09,0x14,0xff}// Tx1+ARDF
//#rom 0x2100={0x00,0x0b,0x38,0x13,0x0f,0x04,0xff} // Tx1+G3ZOI
//#rom 0x2100={0x00,0x08,0x07,0x3f,0x0b,0x05,0x08,0xff} // Tx1+SM0GMS
#rom 0x2100={0x00,0x08,0x07,0x3f,0x10,0x16,0x12,0xff} // Tx1+SM0HPL
...
int8 const timeLimit=18; // 3 hours (in 10min units): automatic shut down
int8 const comfort=6; // 1 hour (in 10 min units): pre-start check transmissions
...
int8 const id[6] ={0x00,0x02,0x04,0x08,0x10,0x20}; //MO,MOe,i,s,h,5 (classic)
int8 const fo[12]={0x06,0x11,0x15,0x09,0x02,0x14,  //A,B,C,D,E,F (foxOring)
                   0x0b,0x10,0x04,0x1e,0x0d,0x12}; //G,H,I,J,K,L (foxOring)
int8 const de[3]={0x09,0x02,0x00};// DE
...
int8 tmps;
int8 tx_mode=0;
int8 active=0; // time count after delay (in 10min units)
...
//        break;
        ...
        case 1: //
          nfoxes=1; // HPL for continously FoxOring 
          on_time=60;
        break;
...
//        break;
        ...
        case 3: // HPL 3 TX Practice Classic 60 
          nfoxes=3;
          on_time=60;
        break;
...
{
   while(active<timeLimit) // count in timer() function
   {
      timing();
...
void one_letter(char cw_char)//------ send a character -----------
{
